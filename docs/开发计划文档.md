...
- 实现配置数据结构设计
- 使用 electron 安全存储 API 实现配置的保存和读取
- 实现配置加密存储功能，特别是用户凭证
...
- electron-build
- electron-plugin-store (配置存储)
- electron-plugin-window-state (窗口状态保存)
- electron-plugin-fs (文件系统操作)
